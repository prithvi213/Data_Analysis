...
def main():
    ...
    bins = ["Yes", "No"]
    amounts = [yes_count, no_count]
    colors = ['green', 'red']
    plt.figure(1)
    plt.bar(bins, amounts, color=colors)
    plt.xlabel('Is it a directory?')
    plt.ylabel('Count')

    for i, count in enumerate(amounts):
        plt.text(i, count + 0.5, str(count), ha='center', va='bottom', fontsize=12)

    # Step 7: Create the pie chart
    plt.figure(2)
    pie_labels = [elem[0] for elem in data]
    pie_sizes = [elem[1] for elem in data]
